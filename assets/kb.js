...
window.DMF_KB = [
  ...
  },
  {
    ...
    a: "The DMF Brain is a company knowledge engine — a grounding brain that compounds as the company feeds it. It captures from every work surface (coding agents, chat, email, SMS) through one MCP agent protocol; runs staged ingestion (classify, chunk + embed) promoted only through a human review gate; stores an append-only decision ledger plus a pgvector store and a keyword index; and serves hybrid, RRF-fused retrieval back to models through a custom MCP server of nine tools. Watcher agents flag where one project's advance can be reused in another, and a plain-English review spans both the index and the raw ingest."
  },
  {
    q: "How are the agents kept safe / governed?",
    k: ["safe","safety","governance","guardrails","human","approval","spec","langfuse","observability","budgets","retries","contract"],
    a: "Agents are treated as governed infrastructure, not scripts. Every agent shares one spec shape — tools, outcome, guardrails, test suite — and runs on a shared harness with loop guards, budgets, and retries, observed run-by-run in Langfuse. State machines drive the pipelines, and a human approves anything that touches money or customers. That human-approval spine runs through the product work too: nothing reaches a real customer inbox without sign-off, and every run is logged and auditable."
  },
  {
    q: "CRM intelligence layer",
    k: ["crm","salesforce","apex","lwc","flow","cpq","soql","sales","brief","payments","esignature"],
    a: "An AI layer inside the system of record a sales team uses every day. It generates stage-triggered briefs from parallel queries across 15+ objects, exposes a conversational agent with live SOQL-backed tools over real orders, and puts pre-visit intel on the record. It also owns the unglamorous parts — CPQ, payments, e-signature — that make the smart parts trustworthy. Built on Salesforce: Apex, LWC, Flow, CPQ."
  },
  {
  ...
  },
  {
    ...
    k: ["voice","phone","agent","elevenlabs","twilio","conversation","flows","build spec","microsite"],
    a: "A pipeline that goes from 'who is this business?' to a deployable phone agent. It researches a business, derives what its phone line actually needs to do, and emits the complete build — configs, conversation flows, tools, knowledge base — staged so setup is mechanical and delivered as build-spec microsites. The hard problem it solves: compiling a messy, real business into a correct agent definition."
  },
  {
    ...
    a: "ServiceRelay is the productized booking & intake line from DMF Company: voice agents that answer and book, plus web intake wired to calendars and CRMs, with white-label skins per client. It answers, books, and follows up so a contractor never misses revenue while the operator stays on the roof. Proven inside a live operation before carrying the flag."
  },
  {
    q: "Prospecting & lead engine (DMF product)",
    k: ["lead","prospecting","engine","farm","enrich","score","outreach","state machine","approval","dmf"],
    a: "An agent-run lead engine: source and farm leads, enrich them with agents, score for fit, then reach out — a status-column state machine with a human-approval spine. Nothing reaches a real inbox without sign-off, and every run is logged and auditable."
  },
  {
    q: "Image generation array (DMF product)",
    k: ["image","generation","brand","creative","prompt","multi-model","curation","campaign"],
    a: "A generative image array that turns one creative brief into arrays of on-brand imagery, using style-locked prompt systems and multi-model routing, with human curation before anything ships — volume without the AI-slop look."
  },
  {
    q: "Video generation pipeline (DMF product)",
    k: ["video","generation","script","scenes","broll","ad","cut","short-form","hooks"],
    a: "A script-to-cut ad assembly pipeline: model-written scripts become generated scenes and b-roll, then platform-ready cuts with hooks and captions built in — designed to close the loop against real ad-account performance, not taste."
  },
  {
    ...
    a: "An Ahrefs/Semrush-class lens scoped to a single market: commercial SEO/SERP feeds (rankings, keywords, competitor pages and ads) ingested on schedule into a warehouse with live dashboards showing who ranks, who's spending, and where the gaps are. The big suites cover everyone; this tells one operator what to do next."
  },
  {
    ...
    a: "Full-funnel automation — capture, nurture, campaign production — run by a team of role-agents (strategist, copywriter, creative director, analyst) under one brand memory. One config re-skins the whole suite; the skin-switch is the product."
  },
  {
